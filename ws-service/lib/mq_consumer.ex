defmodule WsServiceWeb.MQConsumer do
  ...
  @cmd_chat_message_delivery_queue System.get_env("CMD_CHAT_MESSAGE_DELIVERY_QUEUE") || "cmd_chat_message_delivery_queue"
  @cmd_chat_message_send_exchange System.get_env("CMD_CHAT_MESSAGE_SEND_EXCHANGE") || "cmd_chat_message_send_exchange"
  @broker System.get_env("BROKER") || "amqp://guest:guest@broker:5672"
  ...
  def handle_cast({:publish, message_map}, %{channel: chan} = state) do
    IO.puts("→ Publicando mensaje en la cola #{@cmd_chat_message_delivery_queue}")
    payload = Jason.encode!(message_map)
    Basic.publish(chan, @cmd_chat_message_send_exchange, "", payload,
      persistent: true,
  ...
  @impl true
  def init(state) do
    case Connection.open(@broker) do
      {:ok, conn} ->
        {:ok, chan} = Channel.open(conn)
        # Queue.declare(chan, @queue, durable: true)
        # Exchange.declare(chan, "logs", :fanout, durable: true)
        # AMQP.Queue.bind(chan, @queue, "logs")
        Basic.consume(chan, @cmd_chat_message_delivery_queue, nil, no_ack: false)
        IO.puts("AMQP: Crea cola #{@cmd_chat_message_delivery_queue} en el canal #{inspect(chan)}")
        {:ok, %{channel: chan}}
      {:error, _} ->
